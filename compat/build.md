...
If you run a demo, for example, `demos/demo_ipc_stack.py`, you will get a dynamic linkage error when IPC is enabled.
Just making the *.so files available in your linking path (by copying the files to your library path, or by adding their directory to `LD_LIBRARY_PATH`) will not solve the problem. Some libraries are imported using relative paths, so you need to ensure your current working directory contains these library files.

TODO: This behavior is unexpected. CMake install configuration may be needed.